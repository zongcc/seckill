...
public class DatagramChannelServerDemo {
    ...
    /*
     * 接收 用receive()读IO
     * 作为服务端一般不需要调用connect()，如果未调用<span style="font-family: Arial, Helvetica, sans-serif;">connect()时调</span><span style="font-family: Arial,
     * Helvetica, sans-serif;">用read()\write()读写，会报java.nio.channels</span>
    ...
    synchronized public void reveice(SelectionKey key) throws IOException {
        ...
        buf2.clear();
        buf2.put(("消息推送内容 abc..UDP是一个非连接的协议，传输数据之前源端和终端不建立连接，当它想传送时就简单地去抓取来自应用程序的数据，并尽可能快地把它扔到网络上。在发送端UDP" +
                        "是一个非连接的协议，传输数据之前源端和终端不建立连接，当它想传送时就简单地去抓取来自应用程序的数据，并尽可能快地把它扔到网络上。在发送端UDP" +
                        "是一个非连接的协议，传输数据之前源端和终端不建立连接，当它想传送时就简单地去抓取来自应用程序的数据，并尽可能快地把它扔到网络上。在发送端@Q")
                        .getBytes());
        buf2.flip();
